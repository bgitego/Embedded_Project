...
#define SYSCTL_RCGC1_UART0      (1U<<0)  // UART0 Clock Gating Control
#define SYSCTL_RCGC2_GPIOA      (1U<<0)  // port A Clock Gating Control
...
#define UART0_RXPCTL     (1U << 0)
#define UART0_TXPCTL     (1U << 4)
...
void UART_Init(void){
  ...
  SYSCTL -> GPIOHBCTL |= (1U << 0);                // 2a. Enable High Speed for GPIOA
...
char character;
  ...
  while(character != CRJ){ // accepts until <enter> is typed
